/* `제약 조건 constraints`
   ...
   4) FOREIGN KEY(참조키) : 다른 테이블의 값을 참조하기 위해 사용
*/   


-- 1) 학생 테이블
CREATE TABLE tbl_student (
	stuno char(7) PRIMARY KEY,   -- PK 제약조건 설정방법 1) 컬럼레벨 
...
-- 참조관계일때 테이블 삭제 
-- 참조관계의 부모테이블 `학생`은 drop 못합니다.
-- 2개의 테이블을 모두 삭제하려면 자식테이블부터 drop하고 부모테이블을 drop하기
DROP TABLE TBL_STUDENT ;		--부모테이블
DROP TABLE TBL_SCORE ;			--자식테이블


-- 참조관계일때 테이블 데이터 삭제 : 참조되는 데이터가 성적테이블에 있으면 학생테이블 데이터를 삭제 못합니다.
...
INSERT INTO tbl_score(stuno,subject,jumsu,teacher,term)
VALUES (NULL,'과학',88,'박세리','2020_1');		--error
