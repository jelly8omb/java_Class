 ...
 * 참고 : 오라클에서는 테이블, 컬럼명 등 식별자 이름에 할수 있는 기호 _ , #, $.
 * */
...
CREATE TABLE tbl# (
   ...
   col3 varchar2(10),
   col4 number(3)
);   -- CREATE 는 1번만 실행합니다. 오라클 객체 생성 이후 변경사항은 ALTER 명령으로 한다.
...
-- 7) 제약조건 제거 : 제약조건에 변경은 없습니다. add 와 drop 만 있습니다.
ALTER TABLE tbl# DROP CONSTRAINT tbl#_notnull3;
ALTER table tbl# DROP CONSTRAINT tbl#_notnull1; 
...
-- alter table 로 col1을 primary key로 변경해보기. (나중에는 alter table 로 fk 추가도 할수 있어야 합니다.)
ALTER TABLE tbl# MODIFY col1 PRIMARY KEY;
-- 기본키가 복합컬럼이라면 제약조건 추가 키워드 써야합니다.
