/*
 * 스키마 : 데이터베이스 스키마(database schema)는 데이터베이스에서 자료의 구조, 자료의 표현 방법, 자료 간의 관계를 형식 언어로 정의한 구조이다.
 * 			ㄴ 풀이 : 사용자가 계정을 받아서 여러가지 방법으로 데이터(자료)를 저장하고 관리하는 공간
 * 						오라클 등 dbms 는 스키마에 여러 객체를 생성하여 사용합니다.
 * 
 * 실제 비지니스 로직을 처리 하는 SQL이 많이 복잡해질 수 있다.(조인 횟수, 서브쿼리 횟수 많아짐)
 * => 자주 사용하면서 일회용으로 처리가 되면 성능에 문제 생김. == 비효율적.
 ...
 * 
 * create view : 권한이  별도로 필요합니다.	=> 관리자 계정 sys 또는 system 계정필요
 * 
 * 1. SQL Plus 에서 grant create view to c##idex; 명령 실행하고
 * 2. 디비버에서 접속 재연결
 * */
