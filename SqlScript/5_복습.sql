...
CREATE TABLE tbl_member (
	...
	join_date DATE default sysdate  	-- 가입날짜 , 날짜형식 : insert 명령은 날짜 패턴 'yyyy-mm-dd' 문자열이 자동으로 날짜형식 변환
...
SELECT * FROM TBL_MEMBER WHERE NAME AND EMAIL ; 
-- 3) 조건 `이름 박사나` 와 일치하는 행의 모든 컬럼 조회
SELECT * FROM TBL_MEMBER WHERE NAME IN ('박사나');
