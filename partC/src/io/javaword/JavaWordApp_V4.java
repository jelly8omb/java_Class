...
/**
 * JavaWordApp_V3 : 단어장 객체를 생성할 때 읽어올 단어장 파일을 꼭 전달해야 합니다.
...
public class JavaWordApp_V4 {
    ...
    private void removeWord() {
        ...
        if(position == -1){         //인덱스 0부터 단어를 찾아서 -1 이면 찾는 단어가 없음.
            System.out.println("삭제할 단어 존재 X");
            return;
        }

        while (position != -1) {
            System.out.println("삭제할 단어 - " + words.getWord(position));
            System.out.print("삭제 : ENTER, 취소 : N ");
        if(System.console().readLine().equals("n")){
            words.remove(position);
            System.out.println("삭제완료");
            position--;  //삭제했을 때는 인덱스값이 position뒤에 요소들은 모두 -1 씩 감소
            }
